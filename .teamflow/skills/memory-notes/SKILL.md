...
Load `basic-memory-cli` first. Use only local CLI commands and project `teamflow`.
...
1. Search before writing:
   ...
   ```bash
   BASIC_MEMORY_SEMANTIC_SEARCH_ENABLED=false basic-memory tool search-notes "authentication decision" --project teamflow --local
   ```
...
3. Update a matching note instead of duplicating it:
   ...
   ```bash
   basic-memory tool edit-note "memory://architecture/authentication" --operation append --content "- [evidence] Refresh rotation test passed." --project teamflow --local
   ```

4. Create a note only when no matching entity exists. Pass multiline bodies through stdin:

   ```bash
   printf '%s\n' '# Authentication Decision' '' 'Use rotating refresh tokens.' '' '## Observations' '' '- [decision] Rotate refresh tokens after every use.' | basic-memory tool write-note --title "Authentication Decision" --folder architecture --type decision --tags "auth,architecture" --project teamflow --local
   ```

5. Traverse relations after writing:

   ```bash
   basic-memory tool build-context "memory://architecture/authentication" --depth 2 --timeframe 30d --project teamflow --local
   ```
