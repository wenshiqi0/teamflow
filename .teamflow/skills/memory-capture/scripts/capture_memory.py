...
def main() -> None:
    ...
    allowed = (project / ".teamflow" / "runs" / "task-receipts").resolve()
    if not receipt.is_file() or allowed not in receipt.parents:
        fail("receipt must be below .teamflow/runs/task-receipts/")
    try:
    ...
    runner = project / ".teamflow" / "skills" / "extract-memory" / "scripts" / "run_pipeline.py"
    if args.resume_run:
