...
def main() -> None:
    ...
    for source in sources:
        ...
        if path.is_file():
            ...
            if re.search(r"(?m)^type:\s*(?:teamflow|workflow)_memory\s*$", "\n".join(text.splitlines()[:20])):
    ...
    if args.stage == "compression":
        ...
        prompt += (
            ...
            f"{(run_dir / '00-evidence-capsule.json').relative_to(project)}. Existing type: teamflow_memory and legacy workflow_memory notes are atomic: "
    ...
    result = subprocess.run(
        [str(teamflow), "run", "--agent", agent, "--model", args.model, prompt],
    ...
    if errors:
        ...
        repair = subprocess.run(
            [str(teamflow), "run", "--agent", agent, "--model", args.model, repair_prompt],
            cwd=project, text=True, capture_output=True, env=os.environ.copy(),
        )
