...
This working repository uses a test-first multi-agent workflow.
...
- `emotional-salience-sensor`, `memory-compressor`, `memory-extractor`, and `memory-formatter` form the serial capture pipeline; GLM-5.2 owns both extraction and final formatting. Models write only below `.workflow/runs/memory/`; deterministic apply writes safe new notes and defers update/supersede proposals.
...
4. Ask `test-writer` to create and validate a test-only patch below `.workflow/runs/test-patches/`.
...
9. Create a verified-task receipt and run `workflow memory-capture`; never automate capture with direct `memory remember`.
...
For command-only requests such as branch creation, committing an already-reviewed diff, pushing, or opening a pull request, use `workflow command` instead of starting this multi-agent sequence.
...
- Put temporary workflow run artifacts below `.workflow/runs/`.
- Wrap delegated code phases with `workflow phase start/finish`; explicit provider timeout, authentication, quota, overload, transport failure, and user cancellation are `BLOCKED`, not implicit retries of the full workflow. There is no local wall-time timeout by default, so silence while a provider queues is not failure evidence.
- Run `workflow source-check` after implementation edits and before test execution.

## Shared memory

Basic Memory data is local under `~/.workflow/memory/`. Use `workflow memory`; do not start cloud sync or a server process.

- Recall at task start and verify every remembered claim.
- Automated task capture must use `workflow memory-capture`. Direct `remember` commands are reserved for explicit manual use, not planner fallback.
